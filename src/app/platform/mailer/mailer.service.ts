...
export class MailerService extends HttpService {
	...
	public query(mailbox: string, option: { start: number, limit: number }, callback: Callback<any>): void {
		this.Encode(option, (error: IErrorObject, optionString: string): void => {
			if (!error) {
				this.http.get(this.endPoint + "/" + this.model + "/auth/query/" + encodeURIComponent(mailbox) + "/" + optionString, this.httpOptions).pipe(retry(3)).subscribe(
					{
	...
	public get(mailbox: string, UID: string, callback: Callback<object>): void {
		this.http.get(this.endPoint + "/" + this.model + "/auth/" + encodeURIComponent(mailbox) + "/" + encodeURIComponent(UID), this.httpOptions).pipe(retry(3)).subscribe(
			{
				next: (result: any): void => {
					if (result) {
						if (result.code === 0) {
	...
	public send(message: any, callback: Callback<object>): void {
		this.http.post(this.endPoint + "/" + this.model + "/auth", message, this.httpOptions).pipe(retry(3)).subscribe(
			{
				next: (result: any): void => {
					if (result) {
						if (result.code === 0) {
	...
	public delete(mailbox: string, UID: string, callback: Callback<any>): void {
		this.http.delete(this.endPoint + "/" + this.model + "/auth/" + encodeURIComponent(mailbox) + "/" + encodeURIComponent(UID), this.httpOptions).pipe(retry(3)).subscribe(
			{
				next: (result: any): void => {
					if (result) {
						if (result.code === 0) {
	...
	public addFlags(mailbox: string, UID: string, flags: string[], callback: Callback<object>): void {
		this.http.put(this.endPoint + "/" + this.model + "/auth/addflags/" + encodeURIComponent(mailbox) + "/" + encodeURIComponent(UID), {flags: flags}, this.httpOptions).pipe(retry(3)).subscribe(
			{
				next: (result: any): void => {
					if (result) {
						if (result.code === 0) {
	...
	public removeflags(mailbox: string, UID: string, flags: string[], callback: Callback<object>): void {
		this.http.put(this.endPoint + "/" + this.model + "/auth/removeflags/" + encodeURIComponent(mailbox) + "/" + encodeURIComponent(UID), {flags: flags}, this.httpOptions).pipe(retry(3)).subscribe(
			{
				next: (result: any): void => {
					if (result) {
						if (result.code === 0) {
